package com.neumob.samples.okhttp;
...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private static final String NEUMOB_CLIENT_KEY = "sjCzDY6j8h8ALfiw";
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.btn_GetJsonData) {
            ...
            try {
                ...
                Log.d("HTTP call End time",":"+getSystemTime());
    ...
    //Asynctask for running the okhttp calls.
        ...
        protected byte[] doInBackground(String... strings) {
            if (choice == 1 || choice == 2) {
                ...
                Request request = builder.build();
                    Log.d("HTTP call Start time",":"+getSystemTime());
                try {
    ...
    //Method for login(sending the json data-username and password)
    ...
    //TO post the JSON data to the server using okhttp
    private static byte[] postJson(String url, String json, Headers.Builder headers) throws IOException {
        ...
        //Creating the okhttpclient using the neumob definition.
        OkHttpClient client = Neumob.createOkHttp3Client();
        Response response = client.newCall(request).execute();

        return response.body().bytes();
    }
    //For Uploading images to the server
    public static MultipartBody uploadRequestBody(String title, String imageFormat, String token, File file) {
        ...
        return new MultipartBody.Builder()
                ...
                .addFormDataPart("filename", title + "." + imageFormat)
